/**
 * 参考：https://github.com/ice-lab/ice-next/blob/master/packages/ice/src/middlewares/mock/getConfig.ts
 */
...
export default function getConfigs(rootDir: string, exclude: string[] = []): MockConfig[] {
  ...
  mockFiles.forEach((mockFile) => {
    ...
    for (const key of Object.keys(config)) {
      const { method, path: url } = parseMockKey(key);
